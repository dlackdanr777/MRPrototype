...
public class BalloonChapterManager : MonoBehaviour, IChapterObject
{
    ...
    [SerializeField] private BalloonHand _leftHand;
    [SerializeField] private BalloonHand _rightHand;
    ...
    private void OnHandTriggerEnterEvnet(BalloonObject balloonObj, Vector3 pushDir)
    {
        // 풍선 타입이 다를 경우
        if (balloonObj.Type != _targetType)
        {
            balloonObj.RigidBody.AddForce(pushDir, ForceMode.Impulse); // 힘을 순간적으로 가함
            return;
        }

        _score++;
        _scoreText.text = "Score: " + _score;
        _touchParticle.transform.position = balloonObj.transform.position + new Vector3(0, 0.2f, 0);
